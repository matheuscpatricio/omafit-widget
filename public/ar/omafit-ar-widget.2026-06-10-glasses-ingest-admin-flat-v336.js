...
/** Compensação de escala no pulso — 0,94 deixava a pulseira grande mesmo no mínimo do slider. */
const OMAFIT_BRACELET_SCALE_BOOST = 0.78;
/** Multiplicador global só pulseira (após fit anatómico). */
const OMAFIT_BRACELET_RUNTIME_DISPLAY_MUL = 0.82;
...
/**
 * Clip geométrico (clippingPlanes): desligado — cortava o anel inteiro / piscava (v159–v160).
 * Oclusão top/bottom usa fade de material por ângulo de câmara (sem clip).
 */
const OMAFIT_BRACELET_HEMISPHERE_CLIP_ENABLED = false;
/** Micro-UX de escala (snapBoost 0.9→1) na pulseira — desligado (piscava ~3×). */
const OMAFIT_BRACELET_MICRO_UX_SCALE_ENABLED = false;
 ...
 */
const OMAFIT_AR_WIDGET_BUILD = "2026-06-10-glasses-ingest-admin-flat-v336";
...
/** Default `data-ar-necklace-neck-wear-along` → ver `resolveNecklaceNeckJawWidthDropMul`. */
...
/** Largura alvo do arco — `omafit-necklace-calibration.js` (≈ 30 cm + calibração visual). */
/**
 * Ajuste fino do lojista — metafield `omafit.ar_calibration.scale` e/ou
 * `data-ar-necklace-scale-mul` (ver `omafit-necklace-calibration.js`, default 1).
...
/** Suavização da posição world do torso (ms). */
...
const OMAFIT_NECKLACE_TORSO_UPDATE_POS_THRESH_M = 0.022;
/** Head guard: movimento norm do mid-ombros para aceitar update (= corpo). */
const OMAFIT_NECKLACE_HEAD_GUARD_BODY_NORM = 0.014;
/** Head guard: movimento norm do nariz Pose sem ombros (= só cabeça). */
...
const OMAFIT_NECKLACE_TORSO_STABLE_JITTER_M = 0.009;
/** zDist plausível (unproject) antes de aceitar freeze. */
const OMAFIT_NECKLACE_TORSO_ZDIST_VALID_MIN = 0.22;
const OMAFIT_NECKLACE_TORSO_ZDIST_VALID_MAX = 1.12;
...
const OMAFIT_NECKLACE_ANCHOR_SCALE_CLAMP_MIN = 8;
const OMAFIT_NECKLACE_ANCHOR_SCALE_CLAMP_MAX = 22;
...
function omafitGlassesFlatModeForceDrawableOnFace(THREE, root, opts = {}) {
  ...
  root.traverse((child) => {
    ...
    for (const mat of mats) {
      ...
      } else if (isRodinLens && preserveRodinGlb) {
        const tx = Number(mat.transmission) || 0;
        if (tx > 0.02) {
          mat.transparent = true;
          mat.opacity = 1;
        } else if (Number(mat.opacity) < 0.05) {
          mat.opacity = 1;
          mat.transparent = false;
        }
      } else if (isRodinLens) {
  ...
  root.traverse((child) => {
    ...
    for (const mat of mats) {
      ...
      if ("opacity" in mat) {
        ...
        if (!isLens && (!Number.isFinite(o) || o < 0.02)) {
          mat.opacity = 1;
          mat.transparent = false;
        }
      }
...
/**
 * Remove overlays v173 (discos mal posicionados).
 * @param {import("three").Object3D} root
 */
function omafitRemoveMonolithicGlassesLensOverlays(root) {
  if (!root?.traverse) return;
  const toRemove = [];
  root.traverse((o) => {
    if (o?.name === "omafit_lens_overlay") toRemove.push(o);
  });
...
/**
 * GLB monolítico sem `lens_glass`: aviso único — regenerar via worker (Reenfileirar no admin).
 * @param {object} st
 * @param {import("three").Object3D} root
 * @param {string} lensType
 */
function omafitWarnMonolithicGlassesLensRegen(st, root, lensType) {
  if (!root?.traverse) return;
  if (st?.monolithicLensRegenWarned || root.userData?.monolithicLensRegenWarned) return;
  const lt = String(lensType || "clear_fake").trim().toLowerCase();
  if (!omafitMonolithicLensOverlayEligible(lt)) return;
  let meshCount = 0;
  root.traverse((o) => {
    if (o?.isMesh && !/omafit_lens_overlay/i.test(String(o.name || ""))) meshCount += 1;
  });
  if (meshCount !== 1) return;
  st.monolithicLensRegenWarned = true;
  omafitRemoveMonolithicGlassesLensOverlays(root);
  console.warn(
    "[omafit-ar] GLB monolítico sem mesh lens_glass — Reenfileirar no admin AR para split omafit_lens",
    { build: OMAFIT_AR_WIDGET_BUILD, lensType: lt, meshes: meshCount },
  );
}

/** @deprecated use omafitRefreshMonolithicGlassesLensOverlay após bind + câmara */
function omafitEnsureMonolithicGlassesLensOverlay(THREE, mesh, lensType, camera) {
  return omafitRefreshMonolithicGlassesLensOverlay(THREE, mesh, lensType, camera);
}
...
/**
 * Material translúcido lite — paridade preview admin (sem transmissão / sem metalness).
 * @param {typeof import("three")} THREE
 * @param {string} lensType
 */
...
/**
 * Aplica translúcido (lite) ou transmissão (physical) só em meshes de lente.
 *
 * @param {typeof import("three")} THREE
 * @param {import("three").Object3D} root
 ...
 */
function omafitApplyGlassesLensAppearance(THREE, root, opts = {}) {
...
function omafitApplyGlassesLensAppearanceWithFallback(THREE, root, opts = {}) {
  ...
  /** GLB monolítico: cutouts via `omafitTryCommitMonolithicLensOverlay` (sem placas). */
  return primary;
}

/**
 * Preserva PBR do GLB Rodin na armação (paridade preview admin `/calibrate`).
 * O path AR lite antigo zerava metalness/roughness e apagava detalhe.
 *
 * @param {typeof import("three")} THREE
 * @param {import("three").Object3D} root
 */
function omafitPrepareGlassesFrameMaterialsForAr(THREE, root) {
  if (!THREE || !root?.traverse) return;
  root.traverse((obj) => {
    if (!obj.isMesh || !obj.material) return;
    const meshName = String(obj.name || "");
...
function omafitNecklaceSuppressFaceDepthOcclusion(mindarThree, exceptMeshes) {
  ...
  const patchMesh = (mesh) => {
    ...
    if (/necklace|colar|glasses-model|omafit-ar-glasses|omafit-ar-necklace/.test(name)) return;
    const pos = mesh.geometry?.attributes?.position;
    const vertCount = pos?.count ?? 0;
    const mats = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    let isOcc = /face|468|occlud|head|facemesh|mindar/.test(name);
    if (!isOcc) {
      for (const mat of mats) {
        if (!mat) continue;
        if (
          mat.colorWrite === false ||
          (mat.depthWrite === true && mat.depthTest === true && !mat.map && mat.opacity >= 0.99)
        ) {
          isOcc = true;
          break;
        }
    ...
    /** Malha densa do MindAR (468) pode não estar em `faceMeshes` no 1.º frame. */
    if (!isOcc && vertCount >= 900) isOcc = true;
    if (!isOcc) return;
    for (const mat of mats) {
      if (!mat) continue;
      mat.colorWrite = false;
  ...
  if (fms?.length) {
    for (let i = 0; i < fms.length; i++) patchMesh(fms[i]);
  }
...
function omafitEnsureNecklaceMeshesRenderable(THREE, root, opts) {
  if (!THREE || !root?.traverse) return;
  const forceFront =
    opts?.forceDepthFront !== false &&
    !/^(0|false|off|no)$/i.test(String(opts?.forceDepthFrontAttr ?? "1").trim());
  const hasEnv = Boolean(opts?.hasSceneEnvironment);
  omafitEnsureGlassesMeshesRenderable(THREE, root);
  root.traverse((child) => {
    if (!child?.isMesh) return;
    child.renderOrder = 96;
    child.frustumCulled = false;
    const mats = Array.isArray(child.material) ? child.material : [child.material];
    for (const mat of mats) {
      if (!mat) continue;
...
function upgradeFaceArEyewearRendering(THREE, root, envTexture, opts = {}) {
  ...
  root.traverse((obj) => {
    ...
    const next = mats.map((m) => {
      ...
      const isLensMesh =
        m.userData?.omafitArLensMaterial ||
        m.userData?.omafitMonolithicLensPatched ||
 ...
 */
function omafitSafeThreeColor(THREE, colorLike, fallbackHex = 0xffffff) {
  if (colorLike && typeof colorLike.clone === "function") return colorLike.clone();
  if (colorLike && Number.isFinite(colorLike.r)) {
    return new THREE.Color(colorLike.r, colorLike.g, colorLike.b);
  }
  ...
  root.traverse((obj) => {
    ...
    const next = mats.map((m) => {
      ...
      if (isGem && (m.isMeshPhysicalMaterial === true || m.isMeshStandardMaterial === true)) {
        const pm = new THREE.MeshPhysicalMaterial({
          color: omafitSafeThreeColor(THREE, m.color),
      ...
      if (
        ...
        const o = m.isMeshPhysicalMaterial === true ? m.clone() : new THREE.MeshPhysicalMaterial();
        if (m.isMeshStandardMaterial === true) {
          try {
            o.copy(m);
          } catch {
            o.color = omafitSafeThreeColor(THREE, m.color);
            o.metalness = Number(m.metalness) || 0.9;
            o.roughness = Number(m.roughness) || 0.12;
            if (m.map) o.map = m.map;
            if (m.normalMap) o.normalMap = m.normalMap;
          }
        }
        o.envMap = envTexture;
      ...
      if (
        ...
        if (m.isMeshStandardMaterial === true) {
          try {
            ...
            o.color = omafitSafeThreeColor(THREE, m.color);
            o.metalness = Number(m.metalness) || 0.9;
            o.roughness = Number(m.roughness) || 0.16;
            if (m.map) o.map = m.map;
            if (m.normalMap) o.normalMap = m.normalMap;
          }
        }
 ...
 */
/**
 * Largura do arco para normalização — `omafit-necklace-calibration.js`.
 * @param {import("three").Vector3} sz
 * @param {number | null} prepSpanM span guardado após center+Tripo
 */
function omafitResolveNecklaceNeckSpanM(sz, prepSpanM) {
  const span = omafitNecklaceArcSpanFromBbox(sz);
  if (Number.isFinite(prepSpanM) && prepSpanM > 0.04) {
    return Math.max(span, prepSpanM);
  }
  return span;
}
 ...
 * @param {typeof import("three")} THREE
 * @param {{ accessoryMeshNormalizeScale: number, necklacePartition: { chain?: import("three").Object3D, pendant?: import("three").Object3D } | null, necklaceSwing?: { refNeckW: number | null } | null }} st
 * @param {import("three").Object3D} glbRoot
 * @param {any} lm metricLandmarks
 * @param {number} cheekNative distância bochechas no espaço nativo MindAR
 * @param {number} metersMul 1 ou 0.01 (cm→m)
 */
function omafitNecklaceMeterToNativeMul(metersMul) {
  const m = Number(metersMul);
  return Number.isFinite(m) && m > 0 && m < 1 ? 1 / m : 1;
}

/**
 * Escala média das colunas de `matrix` (escala MindAR na âncora facial).
 */
function omafitMatrixAverageScale(THREE, matrix) {
...
/**
 * Delta em cm no espaço `metricLandmarks` → posição local sob `anchor.group`.
 * (Não usar coordenadas absolutas — só deslocamento relativo à âncora.)
 */
function omafitNecklaceNativeDeltaToAnchorLocal(
  THREE,
  anchorGroup,
  dxCm,
  dyCm,
  dzCm,
  out,
  anchorScaleSmoothRef,
) {
  let anchorScale = OMAFIT_NECKLACE_ANCHOR_SCALE_FALLBACK;
  if (anchorGroup) {
    anchorGroup.updateMatrixWorld(true);
    const measured = omafitMatrixAverageScale(THREE, anchorGroup.matrixWorld);
    if (measured >= 4) anchorScale = measured;
  }
  if (anchorScaleSmoothRef && typeof anchorScaleSmoothRef === "object") {
    if (anchorScaleSmoothRef.locked === true) {
      const locked = Number(anchorScaleSmoothRef.value);
      if (Number.isFinite(locked) && locked >= 4) anchorScale = locked;
    } else {
      const prev = Number(anchorScaleSmoothRef.value);
      if (!Number.isFinite(prev) || prev <= 0) {
        anchorScaleSmoothRef.value = anchorScale;
      } else {
        anchorScaleSmoothRef.value += (anchorScale - prev) * 0.06;
      }
      anchorScale = anchorScaleSmoothRef.value;
    }
  }
  const inv = 1 / Math.max(anchorScale, 1e-6);
  out.set(dxCm * inv, dyCm * inv, dzCm * inv);
  return anchorScale;
}

/** @deprecated alias — trata o triplo como delta cm, não ponto absoluto. */
function omafitNecklaceCmOffsetToAnchorLocal(THREE, anchorGroup, xCm, yCm, zCm, out) {
  return omafitNecklaceNativeDeltaToAnchorLocal(THREE, anchorGroup, xCm, yCm, zCm, out, null);
}

/**
 * MindAR nativo ≈ cm: GLB em metros precisa de ×100 uma vez.
 * Não depender só de `cheek > 1,2` (smoother/1º frame pode dar ~0,14 → colar invisível).
 */
function omafitNecklaceMetersToCmFactor(cheekNative, metersMul) {
  const m = Number(metersMul);
  if (Number.isFinite(m) && m > 0 && m < 1) return 100;
  const cheek = Number(cheekNative);
  if (Number.isFinite(cheek) && cheek > 1.2) return 100;
  return 1;
}

/**
 * Escala do colar (metros): `(0,3 m / largura_arco_GLB) × mul_loja × calib`.
 * Sem `×100/anchorDiv` — ver `computeNecklaceArDisplayScale`.
 */
function omafitComputeNecklaceDisplayScale(THREE, st, anchorGroup, cheekNative, metersMul) {
  const norm =
    Number.isFinite(st.accessoryMeshNormalizeScale) && st.accessoryMeshNormalizeScale > 0
      ? st.accessoryMeshNormalizeScale
      : OMAFIT_NECKLACE_REFERENCE_WIDTH_M / Math.max(st.necklaceNeckSpanM || 0.48, 1e-4);
  let cheek = cheekNative;
  if (!Number.isFinite(cheek) || cheek <= 1e-5) cheek = 14;
  ...
  const cheekTrackK =
    st.necklaceSwing && Number.isFinite(st.necklaceSwing.refNeckW) && st.necklaceSwing.refNeckW > 1e-5
      ? THREE.MathUtils.clamp(cheek / st.necklaceSwing.refNeckW, 0.92, 1.08)
      : 1;
  const merchantMul =
  ...
  const anchorDivisor = THREE.MathUtils.clamp(
    anchorScale,
  ...
  let scaleTrackK = cheekTrackK;
  if (
    st.necklaceWorldAnchorMode === true &&
    Number.isFinite(st.necklaceRefShoulderSpan3d) &&
    st.necklaceRefShoulderSpan3d > 0.04 &&
    Number.isFinite(st.necklaceShoulderSpan3d) &&
    st.necklaceShoulderSpan3d > 0.04
  ) {
    scaleTrackK = THREE.MathUtils.clamp(
      st.necklaceShoulderSpan3d / st.necklaceRefShoulderSpan3d,
      0.9,
      1.12,
    );
  }
  const scalePack = computeNecklaceArDisplayScale({
    neckSpanM,
    merchantMul,
    cheekTrackK: st.necklaceTorsoFreezeCheekTrack === true ? 1 : scaleTrackK,
    freezeCheekTrack: st.necklaceTorsoFreezeCheekTrack === true,
  });
  const nativeMul = omafitNecklaceMeterToNativeMul(metersMul);
  return {
    totalScale: scalePack.totalScale,
    anchorScale,
    anchorDivisor,
    cheekTrackK,
    merchantMul,
    nativeMul,
    targetArcWidthCm: scalePack.targetArcWidthCm,
    predictedArcWidthCm: scalePack.predictedArcWidthCm,
  };
}

function omafitApplyNecklaceDisplayScale(THREE, st, glbRoot, anchorGroup, lm, cheekNative, metersMul) {
...
/** @deprecated alias */
function omafitComputeNecklaceRigidSlotScale(THREE, st, anchorGroup, cheekNative) {
  const pack = omafitComputeNecklaceDisplayScale(THREE, st, anchorGroup, cheekNative);
  return {
    fitScale: pack.totalScale,
    anchorScale: pack.anchorScale,
    cheekTrackK: pack.cheekTrackK,
    merchantMul: pack.merchantMul,
    nativeCm: cheekNative > 1.2,
    metersToNativeMul: pack.nativeMul,
    targetArcWidthCm: pack.targetArcWidthCm,
    predictedArcWidthCm: pack.predictedArcWidthCm,
  };
}
 ...
 */
/**
 * Valida candidato torácico antes de freeze / wear (evita colar gigante ou fora do corpo).
 */
function omafitNecklaceValidateTorsoFrame(
  THREE,
  st,
  anchorGroup,
  chestWorld,
...
/**
 * Opção C — trapézio só Pose (ombros) → world via câmara Three.js.
 * MindAR fica fora do posicionamento; malha facial só oclusão/escala fallback.
 *
 * @returns {boolean}
 */
function omafitNecklacePoseTorsoToWorld(
  THREE,
  poseLm,
  camera,
  torsoScratch,
...
/** Landmark trapézio (métrico) → world via âncora MindAR (legado; só fora de torsoMode). */
function omafitNecklaceApplyMetricWearToWorld(
  THREE,
  anchorGroup,
  anchorVec,
  neckWearPt,
  ...
  let dy = wearDy;
  let dz = wearDz;
  if (neckWearPt && anchorVec) {
    dx += neckWearPt.x - anchorVec.x;
    dy += neckWearPt.y - anchorVec.y;
    dz += neckWearPt.z - anchorVec.z;
  }
  ...
  outWorld.copy(st.necklaceWearTarget);
  anchorGroup.localToWorld(outWorld);
  return true;
}

/** Mid-ombros em world (Pose + unproject + matrixWorld da câmara). */
function omafitNecklaceShoulderMidWorld(
  THREE,
  poseLm,
  camera,
  torsoScratch,
  ...
  if (
    !omafitComputeNecklaceTorsoAnchorShouldersOnly(
      ...
      {
        aspect,
        mirrorX: mirrorUnprojectX === true,
        zDistSmooth: lockedZDist ? null : undefined,
        lockedZDist: lockedZDist ? Number(lockedZDist) : undefined,
        downOffsetM: 0,
      },
    )
  ) {
...
function omafitNecklaceSetTrapeziusWearVisible(wearGrp, glbRoot, visible) {
  if (wearGrp) wearGrp.visible = visible;
  if (!glbRoot) return;
  glbRoot.visible = visible;
  if (!visible) return;
  glbRoot.traverse((o) => {
    if (o) o.visible = true;
  });
...
/** Métricas dos ombros em coords normalizadas (mesmo espaço que Pose). */
function omafitNecklaceShoulderNormMetrics(poseLm, aspect, mirrorX) {
  ...
  const midY = (lSh.y + rSh.y) * 0.5;
  const dx = (rx - lx) * Math.max(0.05, aspect || 1);
  const dy = rSh.y - lSh.y;
  const spanNorm = Math.hypot(dx, dy);
  if (spanNorm < 0.03) return null;
  const pn = poseLm[0];
  const faceX = pn && mirrorX === true ? 1 - pn.x : pn?.x;
  const faceY = pn?.y;
  return { midX, midY, spanNorm, faceX, faceY };
}

/** Bloqueia update quando só a cabeça mexeu (nariz) sem deslocamento de ombros. */
function omafitNecklaceHeadGuardAllowBodyUpdate(st, metrics) {
  if (st?.necklaceHeadGuard !== true || !metrics) return true;
...
function omafitNecklaceUpdateTorsoRef(st, metrics, zDist) {
  if (!metrics || !st) return;
  if (!st.necklaceTorsoRef) st.necklaceTorsoRef = {};
  const r = st.necklaceTorsoRef;
  r.shMidX = metrics.midX;
  r.shMidY = metrics.midY;
  r.spanNorm = metrics.spanNorm;
  if (Number.isFinite(metrics.faceX)) r.faceX = metrics.faceX;
  if (Number.isFinite(metrics.faceY)) r.faceY = metrics.faceY;
  if (Number.isFinite(zDist) && zDist > 0) r.zDist = zDist;
}

function omafitNecklaceRigidWearStep(THREE, st, wearGroup, target, dtSec) {
  if (!THREE || !st || !wearGroup || !target) return;
  /** Rigid slot: segue o pescoço no mesmo frame (sem lag que parece «estático»). */
  if (st.necklaceRigidSlot !== false) {
    wearGroup.position.copy(target);
    st.necklaceWearPrimed = true;
    return;
  }
  const cur = st.necklaceWearLerp || wearGroup.position;
  if (!st.necklaceWearPrimed) {
    cur.copy(target);
    st.necklaceWearPrimed = true;
    wearGroup.position.copy(target);
    return;
  }
  const tau = Math.max(1e-3, OMAFIT_NECKLACE_RIGID_WEAR_LERP_MS);
...
/**
 * Ponto baixo no peito (oclusor cilíndrico) — abaixo do queixo.
 */
...
/** Lê escala/rotação do metafield; rotação no grupo `bind` (paridade calibração admin). */
function omafitRefreshNecklaceMerchantCalFromCfg(THREE, st, cfgAttr) {
  if (!st) return null;
...
function omafitNecklaceWearAndOrientStep(
  ...
  camera = null,
  videoAspect = 1,
  mirrorUnprojectX = false,
) {
  if (!THREE || !st || !anchorGroup || !wearGrp || !lm) return;
  if (typeof cfgAttr === "function") {
    omafitRefreshNecklaceMerchantCalFromCfg(THREE, st, cfgAttr);
  }
  ...
  const torsoMode = st.necklaceTorsoAnchorMode === true && worldSpace === true;
  const orientGrp = st.necklaceOrientGroup;
  const rigid =
  ...
  const applyTorsoWorldTarget = (posWorld, _quatWorld, isHold, freezeOrient = false) => {
    if (!posWorld) return;
    if (!st.necklaceTorsoPosSmooth) st.necklaceTorsoPosSmooth = new THREE.Vector3();
    if (!st.necklaceTorsoPosPrimed || isHold) {
      st.necklaceTorsoPosSmooth.copy(posWorld);
      st.necklaceTorsoPosPrimed = true;
    } else {
      const tau = Math.max(1e-3, OMAFIT_NECKLACE_TORSO_POS_TAU_MS) * 0.001;
      const a = 1 - Math.exp(-Math.max(0, dtSec) / tau);
      st.necklaceTorsoPosSmooth.lerp(posWorld, a);
    }
    st.necklaceWearTargetWorld.copy(st.necklaceTorsoPosSmooth);
    if (orientGrp) {
      const lockedQuat =
        st.necklaceTorsoWorldLockedQuat && st.necklaceTorsoOrientPrimed
  ...
  /**
   * v143 — trapézio contínuo: Pose nos ombros desde o 1.º frame válido; sem wear facial.
   * Head guard: ignora jitter da cabeça; segue deslocamento real dos ombros.
   */
  if (torsoMode) {
    const poseReady =
      Boolean(camera && st.lastPoseLandmarksNorm && st.poseShoulderOk === true);
    const holdPose =
      !poseReady &&
    ...
    if (!poseReady && !holdPose) {
      omafitNecklaceSetTrapeziusWearVisible(wearGrp, null, false);
      st.necklaceTrapeziusVisible = false;
      st.necklaceLastNeckSource = "torso-hidden-no-shoulders";
      return;
    }

    if (holdPose && !poseReady) {
      ...
      st.necklaceLastNeckSource = "torso-hold-pose-lost";
      return;
    }

    const torsoScratch = st.necklaceTorsoScratch;
    const lockedZ =
      st.necklaceTorsoZLocked === true &&
      st.necklaceTorsoRef &&
      Number.isFinite(st.necklaceTorsoRef.zDist)
        ? st.necklaceTorsoRef.zDist
        : undefined;
    if (!st.necklacePoseChestWorld) st.necklacePoseChestWorld = new THREE.Vector3();
    if (!st.necklacePoseQuatWorld) st.necklacePoseQuatWorld = new THREE.Quaternion();
    if (!st.necklacePoseWearOffCam) st.necklacePoseWearOffCam = new THREE.Vector3();
    st.necklacePoseWearOffCam.set(
      wearDx * fineWorldM,
      wearDy * fineWorldM,
      wearDz * fineWorldM,
    );
    const chestWorld = st.necklacePoseChestWorld;
    const quatWorld = st.necklacePoseQuatWorld;
    let zDistProbe = Number(st.necklaceTorsoZDistSmooth?.value);
    const poseTorsoOk =
      torsoScratch &&
      omafitNecklacePoseTorsoToWorld(
        THREE,
        ...
        quatWorld,
        {
          aspect: videoAspect,
          mirrorX: mirrorUnprojectX === true,
          zDistSmooth: lockedZ ? null : st.necklaceTorsoZDistSmooth,
          lockedZDist: lockedZ,
    ...
    if (poseTorsoOk) {
      zDistProbe = Number(torsoScratch.lockedZDistUsed);
      const scaleProbe = Number.isFinite(st.necklaceLastFitScale)
        ? st.necklaceLastFitScale
        : st.necklaceBootAppliedScale;
      const frameValid = omafitNecklaceValidateTorsoFrame(
        THREE,
        st,
        anchorGroup,
        chestWorld,
      ...
      if (bodyMove) {
        ...
        if (frameValid && metrics) {
          ...
          if (
            ...
            torsoScratch.shoulderSpan3d > 0.04
          ) {
            st.necklaceShoulderSpan3d = torsoScratch.shoulderSpan3d;
            if (!Number.isFinite(st.necklaceRefShoulderSpan3d)) {
              st.necklaceRefShoulderSpan3d = torsoScratch.shoulderSpan3d;
            }
          }
          if (!st.necklaceTorsoZLocked) {
            st.necklaceTorsoZLocked = true;
  ...
  const jawDropMul = resolveNecklaceNeckJawWidthDropMul(
    resolveNecklaceNeckWearAlongFromCfg(cfgAttr),
  );
  clavScratch.neckWear = neckWearPt;
  const neckWearOk =
    clavScratch &&
    neckWearPt &&
    omafitComputeNecklaceNeckWearPoint(
      THREE,
      (idx, o) => omafitMetricLandmarkToVec3(lm, idx, st.lmSmoother, o),
      {
  ...
  /**
   * Contrato rigid slot (default): malha rígida; posição/orientação do pescoço
   * recalculadas em cada frame a partir dos landmarks — sem freeze de wear/orient.
   */
  ...
  if (strictPose && !strictPoseOk) {
    ...
    /**
     * Bootstrap de visibilidade: sem trapézio/torso válido ainda, mantém update facial
     * até o 1º lock torácico; evita colar “sumir” no arranque.
     */
  }
...
/**
 * Lê um landmark MindAR `metricLandmarks` ([x,y,z]) ou smoother → `out`.
 * @param {any} lm
 * @param {number} idx
 * @param {{ get(i: number): { x: number, y: number, z: number } | null } | null} smoother
 ...
 * @returns {boolean}
 */
function omafitMetricLandmarkToVec3(lm, idx, smoother, out) {
...
/** Colar: suaviza posição/rotação da âncora; escala MindAR (~14) intacta (wear cm→local). */
function omafitDampMatrix4PosRotOnly(THREE, out, raw, lambda) {
  const p0 = new THREE.Vector3();
  const p1 = new THREE.Vector3();
  const q0 = new THREE.Quaternion();
  const q1 = new THREE.Quaternion();
  const s1 = new THREE.Vector3();
  const sUnused = new THREE.Vector3();
  raw.decompose(p1, q1, s1);
  out.decompose(p0, q0, sUnused);
  const t = THREE.MathUtils.clamp(lambda, 0, 1);
  p0.lerp(p1, t);
  q0.slerp(q1, t);
  out.compose(p0, q0, s1);
}

/** Escala travada só para conversão wear (não altera `anchor.group.matrix`). */
function omafitNecklaceResolveWearAnchorScale(THREE, anchorGroup, smoothRef) {
  let anchorScale = OMAFIT_NECKLACE_ANCHOR_SCALE_FALLBACK;
  if (anchorGroup) {
    anchorGroup.updateMatrixWorld(true);
    const measured = omafitMatrixAverageScale(THREE, anchorGroup.matrixWorld);
  ...
  if (!smoothRef || typeof smoothRef !== "object") return anchorScale;
  const prev = Number(smoothRef.value);
  if (smoothRef.locked === true && Number.isFinite(prev) && prev >= 4) {
    return prev;
  }
  if (anchorScale >= 4) {
    smoothRef.value = anchorScale;
    smoothRef.locked = true;
  } else if (!Number.isFinite(prev) || prev <= 0) {
    smoothRef.value = OMAFIT_NECKLACE_ANCHOR_SCALE_FALLBACK;
  }
  return smoothRef.value;
}
 ...
 */
function omafitGlassesNormalizeMindarAnchorMatrix(matrix, dec, lm, opts, THREE) {
...
/**
 * Meio dos olhos em anchor-local: metric→world via PnP bruto (`rawPoseMat`), depois
 * inv(âncora renderizada). Compensa lag One-Euro na rotação vs posição PnP (v328).
 */
function omafitGlassesEyeMidAnchorLocalFromRawPose(
  THREE,
  lm,
  smoother,
  rawPoseMat,
  renderedAnchorMatWorld,
  scratch,
  out,
) {
  if (!THREE || !lm || !rawPoseMat || !renderedAnchorMatWorld || !out) return false;
  const sc = scratch || {};
  ...
  omafitMetricLmToFaceWorldPoint(sc.eyeMidM, rawPoseMat, sc.eyeMidW);
...
function omafitEnhanceFaceGlbPbrResponse(THREE, root) {
  ...
  root.traverse((o) => {
    ...
    for (const m of mats) {
      ...
      if (m.userData?.omafitArLensMaterial) continue;
      if (m.userData?.omafitMonolithicLensPatched) continue;
...
function clearBraceletHemisphereClipOnMaterials(materials) {
  if (!materials?.length) return;
  for (let i = 0; i < materials.length; i++) {
    const m = materials[i];
    if (!m || typeof m !== "object") continue;
    m.clippingPlanes = null;
    if ("clip" in m) m.clip = false;
  }
}
 ...
 */
const OMAFIT_WATCH_CASE_MESH_RE =
...
function setupWatchStrapBiometricGroup(THREE, glbScene, fitSize, localRingR, debug) {
  const caseRe = OMAFIT_WATCH_CASE_MESH_RE;
  const strapRe = OMAFIT_WATCH_STRAP_MESH_RE;
...
async function runArSession({
  ...
  try {
     ...
     *   - glasses → MindAR Face (abaixo, stack legado)
     *   - necklace → MindAR (oclusão) + Pose ombros contínuo (trapézio, v143 head-guard)
    ...
    if (accessoryType === "glasses") {
      ...
      try {
        ...
        if (glassesPreserveRodinGlbLenses && faceHemisphereLight) {
          /** Admin calibrate: sem hemisphere — só ambient + key + fill. */
          faceHemisphereLight.intensity = 0;
          faceHemisphereLight.visible = false;
        }
    ...
    glasses.traverse((child) => {
      ...
      for (const mat of mats) {
        ...
        const isGlassesFrameMesh =
          accessoryType === "glasses" &&
          !omafitIsGlassesLensMeshMaterial(THREE, glasses, mat, child.name, mat.name);
        if (isGlassesFrameMesh) {
          if (!glassesPreserveRodinGlbLenses && "toneMapped" in mat) mat.toneMapped = true;
        } else if (accessoryType !== "necklace" && accessoryType !== "glasses") {
          ...
          const isRodinLensMat =
            accessoryType === "glasses" &&
            omafitIsGlassesLensMeshMaterial(THREE, glasses, mat, child.name, mat.name);
          if (
            !(glassesPreserveRodinGlbLenses && (isRodinLensMat || isGlassesFrameMesh)) &&
        ...
        const shouldStripTransmission =
          accessoryType !== "glasses" ||
          !isLensMesh ||
          (isLensMesh && glassesPreserveRodinGlbLenses
            ? false
            : !glassesPhysicalLenses || glassesRenderFlags.stripTransmission !== false);
    ...
    if (accessoryType !== "necklace" && !glassesIngestPreBake) {
      try {
    ...
    } else if (accessoryType === "glasses" && glassesIngestPreBake) {
      try {
        console.log(
          "[omafit-ar] glasses GLB ingest — skip bake/flatten (paridade preview GLB intacto)",
    ...
    if (
      ...
      try {
        if (glassesIngestWidgetFrameTag) {
          console.log(
            "[omafit-ar] glasses GLB ingest (omafit_ar_canonical) — hierarquia intacta, sem remap runtime",
            { build: OMAFIT_AR_WIDGET_BUILD, hasOmafitCanonicalNode },
          );
    ...
    if (accessoryType === "glasses" && glassesIngestWidgetFrameTag) {
      ...
      try {
        if (glassesIngestAdminPreviewIntact) {
          ...
          console.log(
            "[omafit-ar] glasses ingest → admin preview intact (GLB intacto, zero mutação)",
            {
              build: OMAFIT_AR_WIDGET_BUILD,
              ...glassesIngestPrep,
    ...
    let glassesIngestWorldMeshMaxDimM = 0;
    if (accessoryType === "glasses") {
    ...
    if (accessoryType === "glasses") {
      ...
      if (accessoryType === "glasses" && glassesIngestWidgetFrameTag) {
        glasses.updateMatrixWorld(true);
        glassesIngestWorldMeshMaxDimM = omafitGlassesIngestMeshWorldMaxDimM(THREE, glasses);
        ...
        try {
          console.log("[omafit-ar] glasses ingest bbox pós-center (preview parity)", {
            ...
            worldMeshMaxDimM: Number(glassesIngestWorldMeshMaxDimM.toFixed(5)),
            intrinsicMeshSpanM: Number(glassesIngestIntrinsicMeshSpanM.toFixed(5)),
    ...
    } else if (glassesCanonicalBlenderExport && accessoryType !== "glasses") {
      try {
        console.log(
    ...
    if (accessoryType === "necklace") {
      try {
        console.log("[omafit-ar] colar: calibração tamanho (metafield)", {
          scaleMul: readNecklaceMerchantScaleMul(),
          cal: normalizeNecklaceMerchantCalibration(
            parseOmafitCalibrationRaw(arCfg?.dataset?.arOmafitCalibration || "") ||
              initialFaceCal,
          ),
        });
    ...
    /** Colar: partition opcional (default off — evita reparent que quebra escala/visibilidade). */
    let necklacePartition = null;
    if (
      accessoryType === "necklace" &&
      /^(1|true|yes|on)$/i.test(String(cfgAttr("arNecklacePartition", "0")).trim())
    ) {
      try {
        necklacePartition = omafitPartitionNecklaceChainPendant(THREE, glasses);
        if (necklacePartition?.chain && necklacePartition.chain.children.length === 0) {
          necklacePartition = null;
        }
      } catch (e) {
    ...
    let necklaceBootAppliedScale = NaN;
    let necklaceScaleMulCfg = OMAFIT_NECKLACE_DEFAULT_SCALE_MUL;
    if (accessoryType === "necklace") {
      necklaceScaleMulCfg = readNecklaceMerchantScaleMul();
    }
    if (accessoryType === "necklace") {
      const szNeckFit = new THREE.Vector3();
      new THREE.Box3().setFromObject(glasses).getSize(szNeckFit);
      necklaceNeckSpanM = omafitResolveNecklaceNeckSpanM(szNeckFit, necklaceArcSpanPrepM);
    ...
    if (accessoryType === "glasses") {
      if (!glassesManualMindarRig) {
        if (glassesSimpleFaceOnly) {
          ...
          const ingestPlan = resolveGlassesIngestDisplayScale({
            ...
            worldMeshMaxDimM: glassesIngestWorldMeshMaxDimM,
            hasOmafitCanonicalNode,
            ingestWidgetFrame: true,
      ...
      const bootPack = omafitComputeNecklaceDisplayScale(
        THREE,
        {
          accessoryMeshNormalizeScale,
          necklaceNeckSpanM,
      ...
      if (necklacePartition?.chain?.children?.length) {
        ...
        if (necklacePartition.pendant) {
          necklacePartition.pendant.scale.setScalar(bootTotal);
        }
      } else {
        glasses.scale.setScalar(bootTotal);
      }
      necklaceBootAppliedScale = bootTotal;
    } else {
    ...
    if (accessoryType === "glasses") {
      if (glassesAdminParityFlat) {
        ...
        let adminMeshScaleInit;
        let displayScaleInit;
        let autoFitFlat = 1;
        if (glassesIngestCanonicalPreScaled) {
          const ingestPlan = resolveGlassesIngestDisplayScale({
            intrinsicMeshSpanM: glassesIngestIntrinsicMeshSpanM,
          ...
          autoFitFlat = ingestPlan.autoFitBase;
          adminMeshScaleInit = Number(mcFlat?.scale) > 0 ? Number(mcFlat.scale) : 1;
          displayScaleInit = ingestPlan.displayScale;
        } else {
          autoFitFlat = resolveGlassesCalibScaleBase({
            bboxWidthLocal: glassesMeshScaleBboxWidth,
          });
          adminMeshScaleInit = clampGlassesDisplayMeshScale(
            resolveGlassesMerchantMeshScale({
              bboxWidthLocal: glassesMeshScaleBboxWidth,
              merchantScaleMul: mcFlat?.scale,
          ...
          displayScaleInit = clampGlassesDisplayMeshScale(
    ...
    try {
      ...
      if (mnx === "1" || mnx === "true" || mnx === "yes") negModelX = true;
        ...
        /** MindAR já espelha o frame selfie — não inverter o GLB (desloca/fora do ecrã). */
        negModelX = false;
      } else {
        negModelX = !disableFaceMirror;
      }
      if (negModelX && !flipSceneX) projectionMirrorFix.scale.set(-1, 1, 1);
        ...
        /** Ry180 + bind canónico: espelho no ramo GLB alinha ponte ao LM168 (paridade v235). */
    ...
    if (accessoryType === "necklace") {
      ...
      try {
        console.log("[omafit-ar] colar: wear + orient(pescoço) + bind(calib)", {
          build: OMAFIT_AR_WIDGET_BUILD,
          canonicalBlenderExport: necklaceCanonicalBlenderExport,
    ...
    const necklaceRigidSlot =
      accessoryType === "necklace" &&
      !/^(0|false|off|no)$/i.test(String(cfgAttr("arNecklaceRigidSlot", "1")).trim());
    const necklaceRigidWearNative =
      accessoryType === "necklace"
        ? parseXyzMeters(
            cfgAttr(
              "arNecklaceRigidWear",
              `${OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.x} ${OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.y} ${OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.z}`,
            ),
            OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.x,
            OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.y,
            OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE.z,
          )
        : null;
    ...
    faceArEnhancementState = {
      ...
      glassesAutoFitScaleRef: glassesIngestCanonicalPreScaled
        ? 1
        : resolveGlassesCalibScaleBase({
        bboxWidthLocal: glassesMeshScaleBboxWidth,
        canonicalBlenderExport: glassesCanonicalBlenderExport,
      ...
      necklaceRigidSlot: accessoryType === "necklace" ? necklaceRigidSlot : false,
      necklaceRigidWearNative: necklaceRigidWearNative
        ? {
            x: necklaceRigidWearNative.x,
            y: necklaceRigidWearNative.y,
            z: necklaceRigidWearNative.z,
          }
        : null,
      necklaceWearTarget:
      ...
      necklaceMeshScaleFrozen: false,
      necklaceScaleStableFrames: 0,
      necklaceScaleFreezeLogged: false,
      necklaceOrientLockedQuat:
        accessoryType === "necklace" ? new THREE.Quaternion() : null,
      necklaceFrozenMeshScale:
        accessoryType === "necklace" ? null : null,
      necklaceWearLockedLocal:
        accessoryType === "necklace" ? new THREE.Vector3() : null,
      necklaceWearSlotOffsetNative: accessoryType === "necklace" ? { x: 0, y: 0, z: 0 } : null,
      necklaceWearAnchorScaleSmooth:
        accessoryType === "necklace" ? { value: OMAFIT_NECKLACE_ANCHOR_SCALE_FALLBACK } : null,
      necklaceOrientLocked: false,
      necklaceAnchorScale: null,
      necklaceLastFitScale: null,
      necklaceTargetArcWidthNative: null,
      necklacePredictedArcWidthNative: null,
      necklaceCheekTrackK: null,
      necklaceMeterToNative: accessoryType === "necklace" ? 0.01 : null,
      necklaceForceDepthFrontAttr:
        accessoryType === "necklace"
      ...
      necklaceBasisScratch:
        accessoryType === "necklace"
          ? {
              ...
              hangNeg: new THREE.Vector3(),
              basisM4: new THREE.Matrix4(),
              qTarget: new THREE.Quaternion(),
              qAnchor: new THREE.Quaternion(),
              qOrient: new THREE.Quaternion(),
              qMerchant: new THREE.Quaternion(),
            }
          : null,
      ...
      anchorDec:
        accessoryType === "glasses" || accessoryType === "necklace"
          ...
          : null,
      anchorRawScratch: accessoryType === "glasses" ? new THREE.Matrix4() : null,
      ...
      necklacePoseStrict:
        accessoryType === "necklace"
          ...
          : false,
      necklaceTorsoAnchorMode:
        accessoryType === "necklace"
          ? !/^(0|false|off|no)$/i.test(String(cfgAttr("arNecklaceTorsoAnchor", "1")).trim())
          : false,
      necklaceTorsoFreezeMode:
        accessoryType === "necklace"
          ? !/^(0|false|off|no)$/i.test(String(cfgAttr("arNecklaceTorsoFreeze", "0")).trim())
          : false,
      necklaceTrapeziusImmediate:
        accessoryType === "necklace"
          ? !/^(0|false|off|no)$/i.test(
              String(cfgAttr("arNecklaceTrapeziusImmediate", "1")).trim(),
            )
          : false,
      necklaceHeadGuard:
        accessoryType === "necklace"
          ? !/^(0|false|off|no)$/i.test(String(cfgAttr("arNecklaceHeadGuard", "1")).trim())
          : false,
      necklaceTorsoRef: accessoryType === "necklace" ? {} : null,
      necklaceTorsoZLocked: false,
      necklaceRefShoulderSpan3d: accessoryType === "necklace" ? NaN : NaN,
      necklaceShoulderSpan3d: accessoryType === "necklace" ? NaN : NaN,
      necklaceTrapeziusVisible: false,
      necklaceTrapeziusActiveLogged: false,
      necklaceTorsoFrozen: false,
      necklaceTorsoFrozenPos:
        accessoryType === "necklace" ? new THREE.Vector3() : null,
      necklaceTorsoFrozenQuat:
        accessoryType === "necklace" ? new THREE.Quaternion() : null,
      necklaceTorsoFrozenShoulderMid:
        accessoryType === "necklace" ? new THREE.Vector3() : null,
      necklaceTorsoFrozenZDist: NaN,
      necklaceTorsoFrozenShoulderMidWorld:
        accessoryType === "necklace" ? new THREE.Vector3() : null,
      necklaceTorsoStableFrames: 0,
      necklaceTorsoCalibMid:
        accessoryType === "necklace" ? new THREE.Vector3() : null,
      necklaceBootAppliedScale:
        accessoryType === "necklace" ? necklaceBootAppliedScale : NaN,
      necklaceTorsoFreezeCheekTrack: false,
      ...
      necklaceTorsoZDistSmooth:
        accessoryType === "necklace" ? { value: NaN } : null,
      necklaceTorsoScratch:
        accessoryType === "necklace"
          ? {
              shL: new THREE.Vector3(),
              shR: new THREE.Vector3(),
              hipL: new THREE.Vector3(),
              hipR: new THREE.Vector3(),
              midSh: new THREE.Vector3(),
              midHi: new THREE.Vector3(),
              chest: new THREE.Vector3(),
              xAxis: new THREE.Vector3(),
              yAxis: new THREE.Vector3(),
              zAxis: new THREE.Vector3(),
              basisM: new THREE.Matrix4(),
              qTorso: new THREE.Quaternion(),
              prevTorsoWorldQuat: new THREE.Quaternion(),
              prevXAxis: new THREE.Vector3(),
            }
          : null,
      necklaceUnprojectMirrorX:
        accessoryType === "necklace" ? !disableFaceMirror : false,
    ...
    if (glassesAdminParityFlat && accessoryType === "glasses") {
      ...
      try {
        ...
        const preserveRodinSync = Boolean(
          glassesPreserveRodinGlbLenses ||
            faceArEnhancementState?.glassesLensLoadState?.preserveRodinGlb,
        );
        ...
        const preserveRodinFallback = Boolean(
          glassesPreserveRodinGlbLenses ||
            faceArEnhancementState?.glassesLensLoadState?.preserveRodinGlb,
        );
      ...
      mindarThree.controller.onUpdate = (payload) => {
        ...
        if (!payload.hasFace) {
          ...
          if (accessoryType === "necklace") {
            ...
            st.necklaceMeshScaleFrozen = false;
            st.necklaceScaleStableFrames = 0;
            st.necklaceScaleFreezeLogged = false;
            st.necklaceFrozenMeshScale = null;
            if (st.necklaceWearAnchorScaleSmooth) {
              st.necklaceWearAnchorScaleSmooth.value = OMAFIT_NECKLACE_ANCHOR_SCALE_FALLBACK;
              st.necklaceWearAnchorScaleSmooth.locked = false;
            }
            if (st.necklaceWearLockedLocal) st.necklaceWearLockedLocal.set(0, 0, 0);
            if (st.necklaceWearSlotOffsetNative) {
              st.necklaceWearSlotOffsetNative.x = 0;
              st.necklaceWearSlotOffsetNative.y = 0;
              st.necklaceWearSlotOffsetNative.z = 0;
            }
            st.necklaceWearPrimed = false;
            if (st.necklaceOrientLockedQuat) {
              st.necklaceOrientLockedQuat.identity();
            }
            if (st.necklaceOrientGroup) {
              st.necklaceOrientGroup.userData.omafitNeckOrientPrimed = false;
            }
            if (st.necklaceSwing) st.necklaceSwing.refNeckW = null;
            st.poseShoulderOk = false;
            st.lastPoseLandmarksNorm = null;
            st.necklaceTorsoEverLocked = false;
            st.necklaceTorsoFrozen = false;
            st.necklaceTorsoStableFrames = 0;
            st.necklaceTorsoPosPrimed = false;
            st.necklaceTorsoOrientPrimed = false;
            if (st.necklaceOrientGroup) {
              st.necklaceOrientGroup.quaternion.identity();
              st.necklaceOrientGroup.updateMatrix();
            }
            if (st.necklaceTorsoScratch?.prevTorsoWorldQuat) {
              st.necklaceTorsoScratch.prevTorsoWorldQuat.identity();
            }
            if (st.necklaceTorsoScratch?.prevXAxis) {
              st.necklaceTorsoScratch.prevXAxis.set(NaN, NaN, NaN);
            }
            st.necklaceTorsoFreezeCheekTrack = false;
            st.necklaceTorsoZLocked = false;
            st.necklaceTorsoFrozenZDist = NaN;
            st.necklaceRefShoulderSpan3d = NaN;
            st.necklaceShoulderSpan3d = NaN;
            st.necklaceTrapeziusVisible = false;
            st.necklaceTrapeziusActiveLogged = false;
            if (st.necklaceTorsoRef) {
              st.necklaceTorsoRef.shMidX = NaN;
              st.necklaceTorsoRef.shMidY = NaN;
              st.necklaceTorsoRef.spanNorm = NaN;
              st.necklaceTorsoRef.faceX = NaN;
              st.necklaceTorsoRef.faceY = NaN;
              st.necklaceTorsoRef.zDist = NaN;
            }
            if (st.necklaceTorsoZDistSmooth) st.necklaceTorsoZDistSmooth.value = NaN;
            st.necklaceStrictPoseLastMs = 0;
        ...
        if (!st.smoothInitialized) {
          ...
          for (let fi = 0; fi < mindarThree.faceMeshes.length; fi++) {
            ...
            if (!st.smoothFaceMats[fi]) st.smoothFaceMats[fi] = new THREE.Matrix4();
            /**
             * v330 flat: malha 468 bruta (paridade com âncora PnP bruta). Damp no
             * occluder desacoplava o referencial face vs óculos em yaw.
             */
            if (st.glassesAdminParityFlat) {
              st.smoothFaceMats[fi].copy(fm.matrix);
            } else {
          ...
          if (accessoryType === "glasses") {
            if (payload.hasFace) {
              try {
                ...
                if (occG > 0) {
                  st.glassesFaceOccMatsPatched = (Number(st.glassesFaceOccMatsPatched) || 0) + occG;
                  console.log("[omafit-ar] óculos: face depth suprimida", {
                    build: OMAFIT_AR_WIDGET_BUILD,
                    matsPatchedThisFrame: occG,
                    matsPatchedTotal: st.glassesFaceOccMatsPatched,
                    faceMeshes: mindarThree?.faceMeshes?.length ?? 0,
                  });
                }
                if (!st.glassesFaceMeshDepthOffLogged) {
                  st.glassesFaceMeshDepthOffLogged = true;
                  console.log("[omafit-ar] óculos: oclusão facial MindAR (1.º frame)", {
                    build: OMAFIT_AR_WIDGET_BUILD,
                    matsPatched: occG,
                    faceMeshes: mindarThree?.faceMeshes?.length ?? 0,
                    glassesAdminParityFlat: st.glassesAdminParityFlat,
            ...
            if (st.glassesAdminParityFlat && glasses && calibRot && wearPosition) {
              ...
              /**
               * v330: baseline v324 — wear só na âncora 168 + calib loja; âncora PnP
               * bruta (v329). Offsets mid(olhos) em calibRot/wear (v327–v329) usavam
               * lmSmoother ou espelho → desync yaw/torto. Proximidade: bridge pivot +
               * faceProximityInsetZ no load (GLB intacto).
               */
              applyGlassesMerchantWearAdminParityFlat(
                wearPosition.position,
                anchor.group.matrixWorld,
              ...
              if (st.glassesIngestCanonicalPreScaled) {
                ...
                autoFitBase = ingestPlan.autoFitBase;
                adminMeshScale =
                  Number(merchantCal?.scale) > 0 ? Number(merchantCal.scale) : 1;
                displayScale = st.glassesForceAnchorUnitScale
                  ? ingestPlan.displayScale
                  : clampGlassesDisplayMeshScale(
                      resolveGlassesMindarLocalMeshScale(
                        adminMeshScale,
                        anchor.group.matrixWorld,
                ...
                st.glassesLastScaleSource = "ingest-pre-scaled(canonical-node)";
              } else {
                autoFitBase =
                ...
                adminMeshScale = clampGlassesDisplayMeshScale(
                  resolveGlassesMerchantMeshScale({
                    bboxWidthLocal:
                      st.glassesMeshScaleBboxWidth ?? st.glassesFrameWidthRawLocal,
                    merchantScaleMul: merchantCal?.scale,
              ...
              if (!st.glassesCalibRuntimeLogged) {
                ...
                console.log("[omafit-ar] glasses admin parity flat (runtime)", {
                  ...
                  merchantCal,
                  adminMeshScale,
                  preScaled: !!st.glassesIngestCanonicalPreScaled,
              ...
              if (!st.glassesWorldBboxLogged) {
                ...
                const expectedWidthM = Number(
                  (
                    st.glassesFrameWidthRawLocal *
                    (st.glassesForceAnchorUnitScale
                      ? adminMeshScale
                      : displayScale * anchorU)
                  ).toFixed(5),
                );
              ...
              if (faceAlignParent && (faceSrc || facePoseFromAnchor)) {
                ...
                if (glassesTrackingWrap && st.glassesSimpleFaceOnly) {
                  ...
                  const applyGlassesMerchantCalibration = () => {
                    ...
                    if (st.glassesIngestCanonicalPreScaled && st.glassesSimpleFaceOnly) {
                      const ingestPlan = resolveGlassesIngestDisplayScale({
                        intrinsicMeshSpanM: st.glassesIngestIntrinsicMeshSpanM,
                        rawBboxWidthM:
                          st.glassesMeshScaleBboxWidth ?? st.glassesFrameWidthRawLocal,
                      ...
                      const adminMeshScale =
                        Number(merchantCal?.scale) > 0 ? Number(merchantCal.scale) : 1;
                      if (st.glassesForceAnchorUnitScale) {
                        displayScale = ingestPlan.displayScale;
                      } else {
                      ...
                      scaleSource = "ingest-pre-scaled(canonical-node)";
                      st.glassesLastAdminMeshScale = adminMeshScale;
                    ...
                    if (useV160MerchantScale) {
                      ...
                      const el = fa.eyeL;
                      if (
                        pickEyeLm(OMAFIT_FACE_LM_EYE_R_OUT, er) &&
                        pickEyeLm(OMAFIT_FACE_LM_EYE_L_OUT, el)
                      ) {
                        const ipdLandmark = er.distanceTo(el);
                        const faceScale = computeFaceMatrixUniformScale(
                          fa.faceWorld.elements,
                        );
                        displayScale = clampGlassesDisplayMeshScale(
                          computeGlassesSimpleFaceIpdMeshScale({
                            ipdLandmark,
                        ...
                        scaleSource = "ipd-landmark";
                        st.glassesLastIpdLandmark = ipdLandmark;
                        st.glassesLastFaceScale = faceScale;
                      }
                    }
                    }
                    ...
                    if (!st.glassesDrawablePerFrameApplied) {
                      ...
                      try {
                        if (
                          st.glassesPreserveRodinGlbLenses ||
                          st.glassesLensLoadState?.preserveRodinGlb
                        ) {
                          omafitEnsureGlassesMeshesRenderable(THREE, glasses);
                          omafitGlassesFlatModeForceDrawableOnFace(THREE, glasses, {
                            preserveRodinGlb: true,
                  ...
                  }
                  {
                    ...
                    if (!st.positionLogged) {
                      ...
                      console.log("[omafit-ar] glasses calibration v51 (mesh×merchant + wear face-axes m)", {
                        ...
                        ipdLandmark: st.glassesLastIpdLandmark,
                        faceScale: st.glassesLastFaceScale,
                        glassesMeshScale: glasses
                          ? {
                              x: glasses.scale.x.toFixed(4),
                              y: glasses.scale.y.toFixed(4),
                              z: glasses.scale.z.toFixed(4),
                            }
                          : null,
                        skipBridgeTranslate,
                        glassesTrackingWrapPosition: {
                      ...
                      if (!st.glassesWorldBboxLogged && glasses) {
                        ...
                        if (anchor?.group) {
                          anchor.group.updateMatrixWorld(true);
                          const ap = anchor.group.getWorldPosition(new THREE.Vector3());
                          anchorWorldPos = {
                            x: Number(ap.x.toFixed(4)),
                            y: Number(ap.y.toFixed(4)),
                            z: Number(ap.z.toFixed(4)),
                          };
                        }
                        const ndcInfo = cam
                          ? {
        ...
        if (accessoryType === "necklace" && st.necklaceSwing) {
          ...
          if (st.necklaceWearTarget && anchor?.group) {
            const arVideoForPose =
              mindarHost?.querySelector?.("video") || st.arVideo;
            const vw = arVideoForPose?.videoWidth || 0;
            const vh = arVideoForPose?.videoHeight || 0;
            const neckVideoAspect =
              vw > 0 && vh > 0 ? vw / vh : mindarThree.camera?.aspect || 1;
          ...
          const fitScale = omafitApplyNecklaceDisplayScale(
            THREE,
            st,
            glasses,
            anchor?.group,
          ...
          if (!st.necklaceDiagLogged && glasses && anchor?.group) {
            ...
            try {
              ...
              console.log("[omafit-ar] necklace tracking diag (1x)", {
                ...
                wearLockStableFrames: st.necklaceWearLockStableFrames,
                frozenMeshScale: st.necklaceFrozenMeshScale,
                ...
                necklaceArcSpanHorizM: st.necklaceArcSpanHorizM ?? st.necklaceNeckSpanM,
                accessoryMeshNormalizeScale: st.accessoryMeshNormalizeScale,
                rigidWear: st.necklaceRigidWearNative,
                wearPosition: {
                  x: wearGrp.position.x,
                  y: wearGrp.position.y,
                  z: wearGrp.position.z,
                },
                wearCmNative: st.necklaceWearCmNative || null,
                ndc: cam
                  ? { x: ndc.x, y: ndc.y, z: ndc.z, onScreen: Math.abs(ndc.x) <= 1.05 && Math.abs(ndc.y) <= 1.05 && ndc.z > -1 && ndc.z < 1 }
                  : null,
                meshScale: {
                  x: glasses.scale.x,
                  y: glasses.scale.y,
                ...
                totalScale: st.necklaceLastFitScale ?? fitScale,
                anchorScale: st.necklaceAnchorScale,
                necklaceScaleMul: st.necklaceScaleMul,
                targetArcWidthCm: st.necklaceTargetArcWidthCm,
                cheekTrackK: st.necklaceCheekTrackK,
                predictedArcWidthCm: Number.isFinite(st.necklacePredictedArcWidthCm)
                  ? st.necklacePredictedArcWidthCm
                  : null,
                nativeMul: omafitNecklaceMeterToNativeMul(metersMul),
                anchorDivisor: st.necklaceAnchorDivisor,
                meshCount: (() => {
                  let n = 0;
                  glasses.traverse((o) => {
                    if (o?.isMesh) n += 1;
                ...
                partitionChainChildren: partDiag?.chain?.children?.length ?? 0,
                worldPos: { x: wp.x, y: wp.y, z: wp.z },
                clavicle: clavOk
                  ? { x: clavicle.x, y: clavicle.y, z: clavicle.z }
                  : null,
                cheekNative,
                anchorIndex,
              });
    ...
    (async () => {
      try {
        ...
        console.warn(
          `[omafit-ar] PMREM facial (${accessoryType === "necklace" ? "colar" : "óculos"}):`,
          e?.message || e,
        );
      }
    })();
...
async function runHandArSession({
   ...
   */
  const OMAFIT_BRACELET_TRIPO_TARGET_DIAMETER_M = 0.052;
    ...
    glbLoader.load(
      ...
      (gltf) => {
        ...
        if (accessoryType === "bracelet") {
          ...
          clearBraceletHemisphereClipOnMaterials(braceletOcclusionMaterials);
        ...
        if (!handMicroUxDisabled) {
          try {
            ...
            if (accessoryType === "bracelet" && !OMAFIT_BRACELET_MICRO_UX_SCALE_ENABLED) {
              handMicroUx.preparedOpacity = false;
              handMicroUx.introComplete = true;
              handMicroUx.snapBoost = 1;
              handMicroUxWrap.scale.setScalar(1);
  ...
  let stableWatchWantFaceTowardCam = true;
  let pendingWatchWantFaceTowardCam = true;
  let pendingWatchFaceTowardCamFrames = 0;
  ...
  let rafId = 0;
  let missedFrames = 0;
  const MISSED_HIDE_THRESHOLD = accessoryType === "bracelet" ? 14 : 6;
  ...
  function updateAnchorFromHand(lms, dtMs, handLabel) {
    ...
    if (accessoryType === "watch" && triLenPalm > 1e-7) {
      /** +tmpY = dorsal (oposto à palma; invariante L/R). */
      if (tmpY.dot(palmTriN) > 0) tmpY.negate();
      tmpX.crossVectors(tmpY, handZForearm).normalize();
      tmpY.crossVectors(handZForearm, tmpX).normalize();
    }
    ...
    if (accessoryType === "watch") {
      ...
      let nextWantFaceTowardCam = stableWatchWantFaceTowardCam;
      if (dorsalDotCam >= OMAFIT_WATCH_DORSAL_CAMERA_HYST_DOT) {
        nextWantFaceTowardCam = true;
      } else if (dorsalDotCam <= -OMAFIT_WATCH_DORSAL_CAMERA_HYST_DOT) {
        nextWantFaceTowardCam = false;
      }
      if (nextWantFaceTowardCam === stableWatchWantFaceTowardCam) {
        pendingWatchFaceTowardCamFrames = 0;
      } else if (nextWantFaceTowardCam === pendingWatchWantFaceTowardCam) {
        pendingWatchFaceTowardCamFrames += 1;
      } else {
        pendingWatchWantFaceTowardCam = nextWantFaceTowardCam;
        pendingWatchFaceTowardCamFrames = 1;
      }
      if (
        pendingWatchFaceTowardCamFrames >= OMAFIT_WATCH_DORSAL_CAMERA_PERSIST_FRAMES
      ) {
    ...
    if (
      ...
      const wristWidth = w5.distanceTo(w17);
      tmpCamToWrist.subVectors(camera.position, smPos);
      if (tmpCamToWrist.lengthSq() > 1e-10) tmpCamToWrist.normalize();
      else tmpCamToWrist.set(0, 0, 1);
      const viewElev = Math.abs(tmpCamToWrist.dot(smY));
      const topDownK = THREE.MathUtils.clamp((viewElev - 0.38) / 0.52, 0, 1);
      /**
       * Rigid: fade lateral + reforço em vista de cima/baixo (sem clip geométrico).
       */
      const occStrMax = braceletIsRigidSlot
        ? THREE.MathUtils.lerp(0.24, 0.5, topDownK)
        : 0.7;
      ...
      const fade = THREE.MathUtils.clamp(facing, 0.42, 1.0);
      const allowAdaptiveOpacity =
        accessoryType === "bracelet" ||
        handMicroUxDisabled ||
        handMicroUx.introComplete;
      for (let mi = 0; mi < braceletOcclusionMaterials.length; mi++) {
        ...
        if (allowAdaptiveOpacity) {
          ...
          const opacityFloor = braceletIsRigidSlot ? 0.78 : 0.35;
    ...
    if (glbRoot && localInnerR > 1e-6) {
      ...
      let suBase =
        ...
        (accessoryType === "bracelet" ? OMAFIT_BRACELET_RUNTIME_DISPLAY_MUL : 1);
      if (accessoryType === "bracelet") {
        /**
    ...
    if (landmarks && landmarks.length >= 18) {
      ...
      if (!handMicroUxDisabled) {
        try {
          ...
          if (
            accessoryType !== "bracelet" ||
            OMAFIT_BRACELET_MICRO_UX_SCALE_ENABLED
          ) {
      ...
      if (missedFrames > MISSED_HIDE_THRESHOLD) {
        ...
        stableWatchWantFaceTowardCam = true;
        pendingWatchWantFaceTowardCam = true;
        pendingWatchFaceTowardCamFrames = 0;
        if (watchStrapRadial?.strap) {
