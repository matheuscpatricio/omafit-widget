 ...
 */
const OMAFIT_AR_WIDGET_BUILD = "2026-05-20-ar-widget-v76";
...
/**
 * Escala no mesh (GLB em m): `(0,36/arco) × 100 / anchor` ≈ 4–6 → ~36 cm no pescoço.
 * Wear em cm usa `/anchor` à parte (v73). Clamps evitam gigante (~74) e invisível (~0,05).
 */
const OMAFIT_NECKLACE_RIGID_SCALE_MIN = 3.6;
const OMAFIT_NECKLACE_RIGID_SCALE_MAX = 6.4;
...
function omafitApplyNecklaceTripoBind(THREE, root) {
  ...
  if (sz.y >= sz.x * 1.15 && sz.y >= sz.z * 1.15) {
    /** Tripo vertical (Y↑): deitar no plano do pescoço + Ry 180° (evita colar invertido). */
...
/**
 * Largura do arco no plano do pescoço: max(X,Z) pós-bind Tripo (ignora Y ~1 m de profundidade).
 * Evita span ~0,07 m no eixo fino com `predictedArcWidthCm` enganoso a 36 cm.
 */
function omafitNecklaceHorizontalArcSpanFromSize(sz) {
  ...
  if (horiz > 0.04 && horiz <= OMAFIT_NECKLACE_MAX_ARC_SPAN_M) return horiz;
  const inBand = [x, y, z]
    .filter((v) => Number.isFinite(v) && v > 0.04 && v <= OMAFIT_NECKLACE_MAX_ARC_SPAN_M)
    .sort((a, b) => a - b);
  if (inBand.length > 0) return inBand[inBand.length - 1];
  return OMAFIT_NECKLACE_REFERENCE_WIDTH_M;
}

/** @deprecated use horizontal span; mediana confundia eixo fino com arco. */
function omafitNecklaceArcSpanFromSize(sz) {
  return omafitNecklaceHorizontalArcSpanFromSize(sz);
}

/**
 * Largura do arco para normalização — ignora profundidade Tripo (~1 m).
...
function omafitResolveNecklaceNeckSpanM(sz, prepSpanM) {
  const horiz = omafitNecklaceHorizontalArcSpanFromSize(sz);
  if (Number.isFinite(prepSpanM) && prepSpanM > 0.04 && prepSpanM <= OMAFIT_NECKLACE_MAX_ARC_SPAN_M) {
    return Math.max(horiz, prepSpanM);
  }
  return horiz;
}
...
/**
 * Deslocamento em cm (landmarks nativos) → posição local sob `anchor.group`.
 * A âncora MindAR traz escala ~14–16; sem dividir, wear −5,8 cm vira ~−93 no mundo (off-screen).
 */
function omafitNecklaceCmOffsetToAnchorLocal(THREE, anchorGroup, xCm, yCm, zCm, out) {
  ...
  out.set(xCm * inv, yCm * inv, zCm * inv);
  return anchorScale;
}
...
function omafitComputeNecklaceDisplayScale(THREE, st, anchorGroup, cheekNative, metersMul) {
  ...
  /** GLB em m → cm (×100); `/anchor` compensa amplificação da âncora facial no mundo. */
  ...
  /** Largura estimada no mundo (cm): `totalScale` já inclui m→cm e `/anchor`. */
  const predictedArcWidthCm = neckSpanM * totalScale * anchorDivisor;
  return {
    totalScale,
    anchorScale,
    anchorDivisor,
  ...
  try {
    ...
    if (!glassesCanonicalBlenderExport) {
      if (accessoryType === "necklace") {
        ...
        necklaceArcSpanPrepM = omafitNecklaceHorizontalArcSpanFromSize(szNeck);
        try {
    ...
    /** Slot directo na âncora 152 (sem wearPosition/calibRot/swing). */
    ...
    if (accessoryType === "necklace") {
      ...
      necklaceWearGroup.add(glasses);
      try {
        console.log("[omafit-ar] colar: slot directo na âncora (sem wearPosition/calibRot)", {
          build: OMAFIT_AR_WIDGET_BUILD,
        });
      } catch {
        /* ignore */
    ...
    if (mindarThree.controller && typeof mindarThree.controller.onUpdate === "function") {
      ...
      mindarThree.controller.onUpdate = (payload) => {
        ...
        if (accessoryType === "necklace" && st.necklaceSwing) {
          ...
          if (rigidSlot && st.necklaceWearTarget) {
            const rigid =
              st.necklaceRigidWearNative || OMAFIT_NECKLACE_RIGID_WEAR_DEFAULT_NATIVE;
            const wearCmX = wearPosM.x + rigid.x + fine.x;
            const wearCmY = wearPosM.y + rigid.y + fine.y;
            const wearCmZ = wearPosM.z + rigid.z + fine.z;
            st.necklaceWearCmNative = st.necklaceWearCmNative || { x: 0, y: 0, z: 0 };
            st.necklaceWearCmNative.x = wearCmX;
            st.necklaceWearCmNative.y = wearCmY;
            st.necklaceWearCmNative.z = wearCmZ;
            omafitNecklaceCmOffsetToAnchorLocal(
              THREE,
              anchor?.group,
              wearCmX,
              wearCmY,
            ...
            if (clavOk && clavicle) {
              let dx = clavicle.x - anchorVec.x + fineX;
              let dy = clavicle.y - anchorVec.y + fineY;
              let dz = clavicle.z - anchorVec.z + fineZ;
              ...
              wearGrp.position.copy(st.necklaceWearTarget);
            } else if (clavScratch?.chin && clavScratch?.nose && clavScratch?.down) {
              if (
                ...
                if (lenFb > 1e-5) {
                  ...
                  omafitNecklaceCmOffsetToAnchorLocal(
                    THREE,
                    anchor?.group,
                    wearPosM.x + downFb.x + fineX,
                    wearPosM.y + downFb.y + fineY,
                    wearPosM.z + downFb.z + fineZ,
                    st.necklaceWearTarget,
                  );
                  wearGrp.position.copy(st.necklaceWearTarget);
                }
                ...
                if (vid && vid.readyState >= 2) {
                  ...
                  if (plm?.[OMAFIT_POSE_L_SHOULDER] && plm?.[OMAFIT_POSE_R_SHOULDER]) {
                    ...
                    shoulderRxBlend = THREE.MathUtils.clamp(
                      (pL.y + pR.y) * 0.5 -
                        (lm[OMAFIT_FACE_LM_CHIN][1] + lm[OMAFIT_FACE_LM_NOSE_BRIDGE][1]) * 0.5,
                      -0.2,
                      0.2,
                    );
                  }
            ...
            const pitchMeas = nb.y - chLm.y;
            if (st.necklaceSwing.headPitchNeutral === null) {
              st.necklaceSwing.headPitchNeutral = pitchMeas;
            }
            ...
            if (st.glassesAnatomy) {
              st.glassesAnatomy.rotation.z =
